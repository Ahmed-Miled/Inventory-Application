...
exports.editPartForm = async (req, res) => {
  ...
  try {
    const partResult = await db.query("SELECT * FROM parts WHERE id = $1", [
      partId,
    ]);
    const categories = await db.query("SELECT id, name FROM categories");
    const brands = await db.query("SELECT id, name FROM brands");
...
exports.createPart = async (req, res) => {
  ...
  if (
    ...
    !manufacturer || !part_number || !brand_id
  ) {
  ...
  try {
    await db.query(
      `
      INSERT INTO parts (name, description, price, quantity, category_id, manufacturer, part_number, brand_id)
      VALUES ($1, $2, $3::numeric, $4::int, $5::int, $6, $7, $8::int)
      `,
      [
        name.trim(),
        description.trim(),
...
exports.editPartForm = async (req, res) => {
  const partId = parseInt(req.params.id);
  if (isNaN(partId)) return res.status(400).send("Invalid part ID");

  try {
    const partResult = await db.query("SELECT * FROM parts WHERE id = $1", [partId]);
    if (partResult.rows.length === 0) return res.status(404).send("Part not found");
    ...
    res.render("parts/form", {
      title: "Edit Part",
      formAction: `/parts/${partId}`,
      buttonLabel: "Update Part",
...
exports.updatePart = async (req, res) => {
  ...
  if (
    isNaN(partId) || !name || !description || !price || !quantity ||
    !category_id || !manufacturer || !part_number || !brand_id
  ) {
    return res.status(400).send("All fields are required.");
  }
  ...
  try {
    await db.query(
      `
      UPDATE parts
      SET name = $1, description = $2, price = $3::numeric, quantity = $4::int,
          category_id = $5::int, manufacturer = $6, part_number = $7, brand_id = $8::int
      WHERE id = $9
      `,
      [
        name.trim(),
